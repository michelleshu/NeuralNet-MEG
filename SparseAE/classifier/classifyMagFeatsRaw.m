function classifyMagFeats(subject, dataFile, resDir, sem_matrix_file)
% Input Parameters:
...
dataMin = min(min(min(struct.data)));
if dataMin < 0 
    struct.data = struct.data + abs(dataMin);
else 
    struct.data = struct.data - dataMin;
end

dataMax = max(max(max(struct.data)));
struct.data = struct.data ./ dataMax .* 0.8;
struct.data = struct.data + 0.1;
% End normalize
...
for tr = 1:num_trials,
    ...
    doOneCrossValNoZ(struct.data, sem_matrix, folds, numFolds, numWords, ...
        sprintf('%s/%s_raw_avrg_%i.mat', res_sub_dir, subject, tr), ...
        zscore_folds, 'cosine');
end
