K_options = [80, 90];
R_options = [60, 80, 100, 120, 140, 160];
...
for k_index = 1 : numel(K_options)
    ...
    for r_index = 1 : numel(R_options)
        ...
        C_acc(k_index, r_index) = ones(3);
        D_acc(k_index, r_index) = ones(4);
        E_acc(k_index, r_index) = ones(5);
        F_acc(k_index, r_index) = ones(6);
        G_acc(k_index, r_index) = ones(7);
        ...
        C_acc(k2, r_index) = twos(3);
        D_acc(k2, r_index) = twos(4);
        E_acc(k2, r_index) = twos(5);
        F_acc(k2, r_index) = twos(6);
        G_acc(k2, r_index) = twos(7);
